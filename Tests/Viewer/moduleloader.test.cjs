...
test('ASL compilation awaits side-effect imports before registration',async()=>{
 const {transformSync}=require('../../Viewer/assets/node_modules/@babel/core');
 const plugin=require('../../Viewer/assets/asl-babel-plugin.js');
 const compiled=transformSync('import "./map.js"; globalThis.registerFloor();',{plugins:[plugin],configFile:false,babelrc:false}).code;
 const AsyncFunction=Object.getPrototypeOf(async function(){}).constructor;
 const order=[];
 await new AsyncFunction('require','globalThis',compiled)(async(source,type)=>{assert.equal(source,'./map.js');assert.equal(type,'asl');await Promise.resolve();order.push('map');},{registerFloor:()=>order.push('floor')});
 assert.deepEqual(order,['map','floor']);
});

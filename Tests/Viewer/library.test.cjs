...
test('folder library watches nested additions/changes/deletions and preserves favorites/viewing metadata/notes', async () => {
    ...
    try {
        ...
        await Promise.all([
            preferences.saveFile({ path: file, favorite: true, viewedAt: 123456, duration: 234567, identity: 'session:test' }),
        ...
        assert.equal(state.files[0].viewedAt, 123456);
        ...
        assert.throws(() => preferences.saveFile({ path: imported, duration: NaN }));
    } finally {
        await library.close();

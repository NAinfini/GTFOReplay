...
const finderStyle = Style(({ css }) => {
    const filters = css.class`display:grid;gap:12px;padding:14px 0;border-bottom:1px solid #303944;`;
    const field = css.class`display:grid;gap:6px;color:#a7b1c1;min-width:0;`;
    const row = css.class`display:flex;align-items:center;justify-content:space-between;gap:10px;`;
    ...
    const dom = html<Mutable<Finder & { search: HTMLInputElement; reset: HTMLButtonElement }>>`
        <div class="${style.wrapper}">
            ...
            <div class="${finderStyle.filters}">
                <label class="${finderStyle.field}">${uiText("Name or number")}<input m-id="search" class="${style.search}" type="search" spellcheck="false" autocomplete="off" /></label>
                ...
                <div class="${finderStyle.field}"><span>${uiText("Sort by")}</span>${sort}</div>

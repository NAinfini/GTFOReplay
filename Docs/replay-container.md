...
| 28 | uint32 | 1: first/header block; 0: one or more ticks; 2: completion record; 4: embedded diagnostics |
...
On teardown, the recorder writes one Brotli-compressed UTF-8 JSON diagnostics block before the completion record. It uses the same CRC32 and chunk header, but does not advance the logical offset or enter the playback/live stream. Its timestamp equals the last stored tick, and decoded JSON is limited to 1 MiB. No gameplay block may follow it. A failed capture can contain diagnostics without completion; a failure before the replay header can contain only the container magic and diagnostics. Disk failures and process crashes may prevent the report from being stored. Recording no longer creates a `.diagnostics.json` sidecar.

The report contains `SchemaVersion`, `SessionId`, `ClosedUtc`, `Failure`, sampling costs and queue/allocation counters. `RawBytes`, `FileBytes` and `StoredRatio` describe the stored gameplay data before the diagnostics/completion overhead. The Viewer shows a human-readable summary and the structured report under Recording diagnostics, and exports JSON together with Viewer errors when requested. Absent diagnostics remain unknown; they do not imply success.

For automated inspection, build `Viewer/electron`, then run `node Tools/inspect-recording.cjs "recording.gtfo"`. Standard output is JSON with container status and embedded diagnostics. Exit codes: 0 for a complete recording without a reported capture failure; 1 for incomplete recordings or reported failures; 2 for unreadable/invalid input. This reads diagnostics without loading gameplay models, including files whose replay header was never completed. Copying the `.gtfo` alone preserves the report. Pair updated Recorder and Viewer builds; earlier Viewers do not understand diagnostics blocks.

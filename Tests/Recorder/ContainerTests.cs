...
internal static class ContainerTests {
    ...
    public static async Task Run() {
        ...
        long failedLength = failedDisk.Length;
        try { await failedWriter.WriteDiagnostics("{}"); throw new Exception("failed writer appended diagnostics"); } catch (IOException) { }
        Check(failedDisk.Length == failedLength, "diagnostics retried a partial disk write");
        using var metadataDisk = new MemoryStream();
        var metadataWriter = new ReplayContainer(metadataDisk);
        await metadataWriter.WriteFrame(Frame(0));
        await metadataWriter.WriteDiagnostics("{\"Failure\":\"capture failed\"}");
        Check(metadataWriter.WrittenBytes == 128, "metadata advanced playback offset");
        try { await metadataWriter.WriteFrame(Frame(100)); throw new Exception("capture continued after diagnostics"); } catch (InvalidOperationException) { }
        using var oversizedDisk = new MemoryStream();
        var oversizedWriter = new ReplayContainer(oversizedDisk);
        try { await oversizedWriter.WriteDiagnostics(new string(' ', ReplayContainer.MaxDiagnosticsSize + 1)); throw new Exception("oversized diagnostics accepted"); } catch (InvalidDataException) { }
        Check(oversizedDisk.Length == 0, "invalid diagnostics partially written");

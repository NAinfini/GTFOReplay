...
export function describeParticipants(event: IndexedEvent, players: ReadonlyMap<number, Player>, enemies: ReadonlyMap<number, { type?: { id: number } }>, detonations: ReadonlyMap<number, { snet: bigint }>, enemyNames: ReadonlyMap<number, string>) {
    ...
    if (kind === 'Vanilla.Enemy.Alert') {
        ...
        if (data.type === 'Explosive') {
            const mine = detonations.get(data.source as number);

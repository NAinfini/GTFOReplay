...
Module._load = function(name, ...args) {
    ...
    return name !== 'electron' ? value : {...value, BrowserWindow:new Proxy(value.BrowserWindow,{construct(Target,[options]) {
        return new Target({...options,show:false,width:1440,height:900,webPreferences:{...options.webPreferences,backgroundThrottling:false}});
    }})};
};
let finished = false;
    ...
    contents.once('did-finish-load',async()=>{
        try {
            const result = await contents.executeJavaScript(`(async()=>{
                ...
                await until(()=>app.profile(),'Profile did not load',45000);
                ...
                await wait(800);
                controls.followPlayer(); view.time(downed[0].time-3000);
                ...
                await until(()=>controls.targetName()===downed[0].participants.find(p=>p.type==='player').name && controls.slot!==undefined,'Automatic camera missed player-down event',5000);
                if(!controls.autoCamera() || view.pause()) throw new Error('Automatic framing changed playback mode.');
                if(controls.targetSlot()!==undefined) throw new Error('Automatic subject leaked into manual player selection.');
                if(view.time()>=downed[0].time) throw new Error('Automatic coverage was not established before the event.');
                await until(()=>document.querySelector('.follow-field [role=combobox]')?.textContent.includes('Auto events'),'Camera dropdown left automatic mode');
                view.pause(true);
                ...
                return {downedEvents:downed.length,enemy:enemy.name,leadIn:3000,followed:controls.targetName(),automaticDowned:true,anticipated:true,automaticDropdown:true,releasePreservesPose:true,reviveTypesDistinct:true};
            })()`);

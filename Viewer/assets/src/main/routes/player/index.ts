...
export const Player = () => {
    interface Player {
        refresh: () => void;
        open: (path?: string, resume?: boolean) => Promise<void>;
    ...
    const saveProgress = () => {
        ...
        if (!restored || !dom.path || !replay?.identity || !replay.complete) return;
        const time = Math.floor(dom.view.time()), duration = replay.length();
        const key = `${dom.path}:${time}:${duration}`;
        if (key === lastSaved) return;
        lastSaved = key;
        void window.api.invoke("saveReplayProgress", dom.path, replay.identity, time, duration).catch(error => {
            lastSaved = "";
            console.error("Could not save playback position:", error);
            dom.view.addLog({ message: String(error), verbose: String(error), type: "error" });
        });
    };
    setInterval(saveProgress, 5000);
    window.addEventListener("beforeunload", saveProgress);
    dom.open = async function open(path?: string, resume = true) {
        const generation = ++opening;
        return openingTask = openingTask.catch(() => {}).then(async () => {
        if (generation !== opening) return;
        saveProgress(); restored = false; lastSaved = "";
        ...
        const previous = path !== undefined && resume ? await window.api.invoke("replayProgress", path) : undefined;
        if (generation !== opening) return;
        ...
        if (clip) {
            ...
            this.view.time(previous?.resume ?? replay.startTime); restored = true;
            app.load(this); saveProgress();
            return;
        }
        ...
        this.parser.addEventListener("eoh", () => {
            ...
            headerReady = true;
            this.view.ready();
            if (previous?.resume) this.view.pause(true);
            app.load(this);
        });
        this.parser.addEventListener("end", async () => {
            ...
            try {
                ...
                if (headerReady && previous?.resume) this.view.time(previous.resume);
                restored = headerReady;
                saveProgress();
